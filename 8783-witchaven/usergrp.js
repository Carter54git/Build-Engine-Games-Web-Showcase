...
(function (global) {
  ...
  function normalizeMountPath(relPath) {
    ...
    if (leaf.toUpperCase() === 'PIRATE.GRP') {
      return norm.slice(0, norm.length - leaf.length) + 'pirate.grp';
    }
    if (leaf.toUpperCase() === 'SW.GRP') {
      return norm.slice(0, norm.length - leaf.length) + 'sw.grp';
    }
